									...
									const gchar *name_owner, gpointer user_data)
{
	...
	sphone_log(LL_DEBUG, "Using modem: %s\n", private->modem);
	g_free(modems);
...
int ofono_dial(const gchar *dial)
{
	...
	sphone_log(LL_DEBUG, "Dialing number: %s\n", dial);
		...
		void *data)
{
	...
	while (g_variant_iter_next(iter, "{sv}", &key, &var)) {
		if (g_strcmp0(key, "Sender") == 0) {
			...
			sphone_log(LL_DEBUG, "%s: sms time: %s\n", __func__, time);
		}
		g_variant_unref(var);
		g_free(key);
	}
...
int ofono_sms_send(const gchar *to, const gchar *text)
{
	...
	sphone_log(LL_DEBUG, "Sending sms: %s %s\n", to, text);
