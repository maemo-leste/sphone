...
sphone_call_finalize (GObject *object)
{
	...
	sphone_log(LL_DEBUG, "Clear call object");
...
static void _sphone_call_properties_callback(call_property_t property, const gchar *value, void *object)
{
	...
	if(property == CALL_PROPERTY_START_TIME){
		...
		sphone_log(LL_DEBUG, "	  value='%s'",private->call_properties->state);
...
sphone_call_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	...
	sphone_log(LL_DEBUG, "%s: %p", __func__, private);
