...
int store_contact_load_details(store_contact_struct *contact)
{
	...
	sphone_log(LL_DEBUG, "store_contact_load_details %d\n",contact->id);
	if(contact->id<=0)
		return 0;
	...
	if(ret>0){
		do{
			...
			sphone_log(LL_DEBUG, "    Add dial %s\n",dial_entry->dial);
...
static int store_contact_get_id(const gchar *name)
{
	int ret=0;
...
int store_contact_add(const gchar *name)
{
	...
	for(size_t i = 0; i < strlen(name_coded);i++){
...
static int store_contact_dial_delete(gint dialid)
{
	int ret=0;
	if(dialid<1)
		return -1;
