...
sphone_store_tree_model_init (SphoneStoreTreeModel *store)
{
	...
	if(private->filter->query_type==FILTER_QUERY_NONE && store_sql_exec(private->filter->query_count,&stmt,G_TYPE_INVALID)>0){
		...
		sphone_log(LL_DEBUG, "sphone_store_tree_model_init rows_count=%d\n",private->rows_count);
	}
...
sphone_store_tree_model_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_FILTER:
		{
			sphone_log(LL_DEBUG, "Add filter \n");
...
sphone_store_tree_model_get_value (GtkTreeModel *tree_model,GtkTreeIter  *iter,gint column,GValue *value)
{
	...
	if(n>=private->tail_pos){
		sphone_log(LL_DEBUG, "sphone_store_tree_model_get_value: Not in cache, create the cache for 10 more entries\n");
			...
			do{
				...
				if(private->filter->type==FILTER_CALL || private->filter->type==FILTER_SMS){
					...
					if(sqlite3_column_int(stmt,5)==STORE_INTERACTION_DIRECTION_OUTGOING)
						entry->interaction_direction=g_strdup("Outgoing");
					else
						entry->interaction_direction=g_strdup("Incoming");
				}
				...
				if(private->filter->type==FILTER_CALL){
					if(sqlite3_column_int(stmt,6)==STORE_INTERACTION_CALL_STATUS_ESTABLISHED)
						entry->interaction_call_status=g_strdup("Answered");
					else
						entry->interaction_call_status=g_strdup("Missed");
					int col = sqlite3_column_int(stmt,7);
					gchar *duration=g_strdup_printf("%02d:%02d:%02d",col/3600,(col/60)%60,col%60);
					entry->interaction_call_duration=duration;
				}

				//sphone_log(LL_DEBUG, "Loaded name=%s dial=%s photo=%s\n",entry->name,entry->dial, picture);
	...
	if(!entry){
	sphone_log(LL_ERR, "sphone_store_tree_model_get_value: can't get %d element\n",n);
		return;
...
sphone_store_tree_model_finalize (GObject *object)
{
	...
	if(private->cache){
		...
		do{
			...
			g_free(entry->interaction_direction);
			g_free(entry->interaction_call_status);
			if(entry->photo)
