...
static void gui_contact_sms_selected_callback(GtkTreeView *view, GtkLabel *textLabel)
{
	...
	sphone_log(LL_DEBUG, "gui_contact_sms_selected_callback\n");
	if(path){
...
static void gui_contact_book_double_click_callback(GtkTreeView *view, gpointer func_data)
{
	...
	GtkTreePath *path;
	gtk_tree_view_get_cursor(view,&path,NULL);
	sphone_log(LL_DEBUG, "gui_contact_book_double_click_callback\n");
	if(path){
		GtkTreeModel *model=gtk_tree_view_get_model (view);
		GtkTreeIter iter;
		GValue value={0};
...
gint gui_contact_open_by_dial(const gchar *dial)
{
	...
	sphone_log(LL_DEBUG, "Dials %p\n",dials);
	while(dials){
		gchar *tmpdial = ((store_dial_struct*)dials->data)->dial;
		...
		sphone_log(LL_DEBUG, "gui_contact_open_by_dial add dial %s\n", tmpdial);
	...
	if(contact->name!=NULL){		// known contact
		...
		sphone_log(LL_DEBUG, "SPHONE_STORE_TREE_MODEL_FILTER_CALLS_MATCH_CONTACT_ID query='%s'\n",contactid);
		...
		sphone_log(LL_DEBUG, "SPHONE_STORE_TREE_MODEL_FILTER_CALLS_MATCH_DIAL_ID query='%s'\n",dialid);
	...
	if(contact->name!=NULL){		// known contact
		gchar contactid[20];
		sprintf(contactid,"%d",contact->id);
		sphone_log(LL_DEBUG, "SPHONE_STORE_TREE_MODEL_FILTER_SMS_MATCH_CONTACT_ID query='%s'\n",contactid);
		calls = sphone_store_tree_model_new(&SPHONE_STORE_TREE_MODEL_FILTER_SMS_MATCH_CONTACT_ID, contactid);
	}else{
		gchar dialid[20];
		sprintf(dialid,"%d",store_dial_get_id(dial));
		sphone_log(LL_DEBUG, "SPHONE_STORE_TREE_MODEL_FILTER_SMS_MATCH_DIAL_ID query='%s'\n",dialid);
...
gint gui_history_calls(void)
{
	...
	//TODO: replace notification_remove("missed_call.png");
	return 0;
}
